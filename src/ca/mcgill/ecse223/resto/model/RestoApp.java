...
// line 3 "../../../../../RestoAppPersistence2.ump"
// line 6 "../../../../../RestoApp-v7.ump"
public class RestoApp implements Serializable
{
  ...
  public RestoApp(Menu aMenu)
  {
    ...
    takeOuts = new ArrayList<TakeOut>();
  }
  ...
  public Table getTakeOutTable()
  {
    return takeOutTable;
  }

  public boolean hasTakeOutTable()
  {
    boolean has = takeOutTable != null;
    return has;
  }
  ...
  public TakeOut getTakeOut(int index)
  {
    TakeOut aTakeOut = takeOuts.get(index);
    return aTakeOut;
  }

  public List<TakeOut> getTakeOuts()
  {
    List<TakeOut> newTakeOuts = Collections.unmodifiableList(takeOuts);
    return newTakeOuts;
  }

  public int numberOfTakeOuts()
  {
    int number = takeOuts.size();
    return number;
  }

  public boolean hasTakeOuts()
  {
    boolean has = takeOuts.size() > 0;
    return has;
  }

  public int indexOfTakeOut(TakeOut aTakeOut)
  {
    int index = takeOuts.indexOf(aTakeOut);
    return index;
  }
  ...
  public boolean setTakeOutTable(Table aNewTakeOutTable)
  {
    boolean wasSet = false;
    takeOutTable = aNewTakeOutTable;
    wasSet = true;
    return wasSet;
  }
  ...
  public static int minimumNumberOfTakeOuts()
  {
    return 0;
  }
  /* Code from template association_AddManyToOne */
  public TakeOut addTakeOut(int aPhoneNumber, String aTakeOutName, Order aOrder)
  {
    return new TakeOut(aPhoneNumber, aTakeOutName, aOrder, this);
  }

  public boolean addTakeOut(TakeOut aTakeOut)
  {
    boolean wasAdded = false;
    if (takeOuts.contains(aTakeOut)) { return false; }
    RestoApp existingRestoApp = aTakeOut.getRestoApp();
    boolean isNewRestoApp = existingRestoApp != null && !this.equals(existingRestoApp);
    if (isNewRestoApp)
    {
      aTakeOut.setRestoApp(this);
    }
    else
    {
      takeOuts.add(aTakeOut);
    }
    wasAdded = true;
    return wasAdded;
  }

  public boolean removeTakeOut(TakeOut aTakeOut)
  {
    boolean wasRemoved = false;
    //Unable to remove aTakeOut, as it must always have a restoApp
    if (!this.equals(aTakeOut.getRestoApp()))
    {
      takeOuts.remove(aTakeOut);
      wasRemoved = true;
    }
    return wasRemoved;
  }

  public boolean addTakeOutAt(TakeOut aTakeOut, int index)
  {  
    boolean wasAdded = false;
    if(addTakeOut(aTakeOut))
    {
      if(index < 0 ) { index = 0; }
      if(index > numberOfTakeOuts()) { index = numberOfTakeOuts() - 1; }
      takeOuts.remove(aTakeOut);
      takeOuts.add(index, aTakeOut);
      wasAdded = true;
    }
    return wasAdded;
  }

  public boolean addOrMoveTakeOutAt(TakeOut aTakeOut, int index)
  {
    boolean wasAdded = false;
    if(takeOuts.contains(aTakeOut))
    {
      if(index < 0 ) { index = 0; }
      if(index > numberOfTakeOuts()) { index = numberOfTakeOuts() - 1; }
      takeOuts.remove(aTakeOut);
    ...
    {
      wasAdded = addTakeOutAt(aTakeOut, index);
    }
    return wasAdded;
  }
  ...
  public void delete()
  {
    ...
    while (takeOuts.size() > 0)
    {
      TakeOut aTakeOut = takeOuts.get(takeOuts.size() - 1);
      aTakeOut.delete();
      takeOuts.remove(aTakeOut);
    }
  ...
  // line 9 "../../../../../RestoAppPersistence2.ump"
   public void reinitialize(){
    ...
    Table.reinitializeUniqueNumber(this.getCurrentTables());
  }
  ...
  // line 6 "../../../../../RestoAppPersistence2.ump"
